...
function green_farm_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function green_farm_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function green_farm_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['green-farm' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date">' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
				'</abbr>' . 
...
function green_farm_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<span class="cmsmasters_post_author_suf">' . esc_html__('by', 'green-farm') . ' </span>' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'green-farm') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
		...
		if ($cmsmasters_option['green-farm' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
...
function green_farm_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out .= '<span class="cmsmasters_post_category">' . 
				'<span class="cmsmasters_post_cat_suf">' . esc_html__('in', 'green-farm') . ' </span>' . 
			...
			if ($cmsmasters_option['green-farm' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
...
function green_farm_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = green_farm_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['green-farm' . '_blog_post_tag']) {
			$out .= '<span class="cmsmasters_post_tags">' . 
				get_the_tag_list(esc_html__('tags', 'green-farm') . ' ', ', ', '') . 
			'</span>';
		}
...
function green_farm_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['green-farm' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function green_farm_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = green_farm_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = green_farm_get_global_options();
			
			if ($cmsmasters_option['green-farm' . '_blog_post_comment']) {
				$out = green_farm_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function green_farm_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
