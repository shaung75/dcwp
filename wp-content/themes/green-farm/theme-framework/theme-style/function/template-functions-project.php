...
function green_farm_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function green_farm_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function green_farm_project_exc_cont($show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_project_content entry-content">' . "\n" . 
		wpautop(green_farm_excerpt(20, false)) . 
	'</div>' . "\n");
...
function green_farm_project_check_exc_cont() {
	$exc = green_farm_project_exc_cont(false);
...
function green_farm_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out = '<span class="cmsmasters_project_category">' . 
				esc_html__('in', 'green-farm') . ' ' . 
				green_farm_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
			'</span>';
		} elseif ($template_type == 'post') {
			$cmsmasters_option = green_farm_get_global_options();
			
			
			if ($cmsmasters_option['green-farm' . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'green-farm') . ':' . '</div>' . 
...
function green_farm_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['green-farm' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'green-farm') . ':' . '</div>' . 
...
function green_farm_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = green_farm_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = green_farm_get_global_options();
			
			if ($cmsmasters_option['green-farm' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'green-farm') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						green_farm_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function green_farm_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['green-farm' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'green-farm') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function green_farm_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			esc_html__('By', 'green-farm') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'green-farm') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = green_farm_get_global_options();
		...
		if ($cmsmasters_option['green-farm' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'green-farm') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'green-farm') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function green_farm_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = green_farm_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['green-farm' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'green-farm') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function green_farm_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function green_farm_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = green_farm_get_global_options();
	$out = '';
	
	if ( 
		$cmsmasters_option['green-farm' . '_portfolio_project_link'] && 
		$link_text != '' && 
		$link_url != '' 
	) {
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'green-farm') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
...
function cmsmasters_project_more($cmsmasters_id, $show = true) {
	...
	if ($cmsmasters_project_read_more == '') {
		$cmsmasters_project_read_more = esc_html__('Learn More...', 'green-farm');
	}
