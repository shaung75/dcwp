...
if (
	$cmsmasters_option['green-farm' . '_portfolio_project_like'] || 
	$cmsmasters_option['green-farm' . '_portfolio_project_date'] || 
	$cmsmasters_option['green-farm' . '_portfolio_project_cat'] || 
	$cmsmasters_option['green-farm' . '_portfolio_project_comment'] || 
	$cmsmasters_option['green-farm' . '_portfolio_project_author'] || 
	$cmsmasters_option['green-farm' . '_portfolio_project_tag'] || 
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_post_format == 'gallery') {
		...
		green_farm_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-project-full-thumb');
		...
		if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
			...
			green_farm_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
		}
	}
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			green_farm_project_title_nolink(get_the_ID(), 'h2');
		echo '</header>';
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_post_format == 'video') {
			...
			green_farm_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
		}
		...
		if (get_the_content() != '') {
			echo '<div class="cmsmasters_project_content entry-content">' . "\n";
				...
				wp_link_pages(array( 
					'before' => '<div class="subpage_nav">' . '<strong>' . esc_html__('Pages', 'green-farm') . ':</strong>', 
					'after' => '</div>', 
					'link_before' => ' [ ', 
					'link_after' => ' ] ' 
				));
		...
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				...
				echo '<div class="project_details entry-meta">';
					...
					green_farm_get_project_likes('post');
					
					green_farm_get_project_comments('post');
					
					green_farm_get_project_author('post');
					
					green_farm_get_project_date('post');
					
					green_farm_get_project_category(get_the_ID(), 'pj-categs', 'post');
					
					green_farm_get_project_tags(get_the_ID(), 'pj-tags');
					
					green_farm_get_project_features('details', $cmsmasters_project_features, false, 'h5', true);
					
					green_farm_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
					
				echo '</div>';
			}
			
			
			green_farm_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h5', true);
			
			green_farm_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h5', true);
			
			green_farm_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h5', true);
...
if ($cmsmasters_option['green-farm' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['green-farm' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['green-farm' . '_portfolio_project_nav_order_cat'] : false);
	
	green_farm_prev_next_posts($order_cat);
}
