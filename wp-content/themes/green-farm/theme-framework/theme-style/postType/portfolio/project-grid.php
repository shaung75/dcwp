...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && green_farm_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		...
		green_farm_thumb_rollover(get_the_ID(), 'cmsmasters-project-grid-thumb', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		...
		if ($categories) {
			echo '<div class="cmsmasters_project_cont_info entry-meta">';
				...
				green_farm_get_project_category(get_the_ID(), 'pj-categs', 'page');
				
			echo '</div>';
		}
		
		
		$title ? green_farm_project_heading(get_the_ID(), 'h3', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
		
		$excerpt ? green_farm_project_exc_cont() : '';
			...
			echo '<footer class="cmsmasters_project_footer entry-meta">';
				...
				if ($likes || $comments) {
					echo '<div class="cmsmasters_project_info">';
						...
						$likes ? green_farm_get_project_likes('page') : '';
						
						$comments ? green_farm_get_project_comments('page') : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				green_farm_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
