		...
		<div class="comment-text cmsmasters_comment_item_cont">
				...
				<h5 class="cmsmasters_comment_item_title" itemprop="author"><?php 
					...
					if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' )
						if ( wc_customer_bought_product( $comment->comment_author_email, $comment->user_id, $comment->comment_post_ID ) )
							echo '<em class="verified">(' . esc_html__( 'verified owner', 'green-farm' ) . ')</em> ';
				?></h5>
				<?php green_farm_woocommerce_rating('cmsmasters_theme_icon_star_empty', 'cmsmasters_theme_icon_star_full', true, $comment->comment_ID); ?>
			...
			<div itemprop="description" class="description cmsmasters_comment_item_content">
				...
				if ($comment->comment_approved == '0') {
					echo '<p class="meta">' . 
						'<em>' . esc_html__( 'Your comment is awaiting approval', 'green-farm' ) . '</em>' . 
					'</p>';
				}
				?>
			</div>
