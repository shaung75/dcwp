...
function green_farm_woocommerce_register_styles_scripts() {
	// Styles
	wp_enqueue_style('green-farm-woocommerce-style', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-style.css', array(), '1.0.0', 'screen');
	
	wp_enqueue_style('green-farm-woocommerce-adaptive', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-adaptive.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('green-farm-woocommerce-rtl', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-rtl.css', array(), '1.0.0', 'screen');
	}
	
	
	// Scripts
	wp_enqueue_script('green-farm-woocommerce-script', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/js/jquery.plugin-script.js', array('jquery'), '1.0.0', true);
	...
	$cmsmasters_image_sizes = green_farm_get_image_thumbnail_list();
	
	
	wp_localize_script('green-farm-woocommerce-script', 'cmsmasters_woo_script', array( 
...
add_action('wp_enqueue_scripts', 'green_farm_woocommerce_register_styles_scripts');


/* Register Default Woocommerce Theme Sidebar */
function green_farm_woocommerce_the_widgets_init() {
	...
	register_sidebar(
		array(
			'name' => esc_html__('Shop Sidebar', 'green-farm'), 
			'id' => 'sidebar_shop', 
			'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on shop page.', 'green-farm'), 
...
add_action('widgets_init', 'green_farm_woocommerce_the_widgets_init');


function green_farm_woocommerce_demo_store($html, $notice) {
...
add_filter('woocommerce_demo_store', 'green_farm_woocommerce_demo_store', 10, 2);


function green_farm_woocommerce_support() {
...
add_action('after_setup_theme', 'green_farm_woocommerce_support');
