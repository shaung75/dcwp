...
add_action('admin_menu', 'green_farm_add_menu');
add_action('admin_init', 'green_farm_register_settings');
...
function green_farm_get_settings() {
	$output = array();
	
	
	$page = green_farm_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_general_sections();
		$cmsmasters_page_fields = green_farm_options_general_fields();
		$cmsmasters_page_tabs = green_farm_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_font_sections();
		$cmsmasters_page_fields = green_farm_options_font_fields();
		$cmsmasters_page_tabs = green_farm_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_color_sections();
		$cmsmasters_page_fields = green_farm_options_color_fields();
		$cmsmasters_page_tabs = green_farm_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_element_sections();
		$cmsmasters_page_fields = green_farm_options_element_fields();
		$cmsmasters_page_tabs = green_farm_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_single_sections();
		$cmsmasters_page_fields = green_farm_options_single_fields();
		$cmsmasters_page_tabs = green_farm_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'green-farm');
		$cmsmasters_page_sections = green_farm_options_demo_sections();
		$cmsmasters_page_fields = green_farm_options_demo_fields();
		$cmsmasters_page_tabs = green_farm_options_demo_tabs();
...
function green_farm_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'green-farm'), 
		'desc' => 		esc_html__('This is a default description.', 'green-farm'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function green_farm_register_settings() {
	$settings_output = green_farm_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'green_farm_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'green_farm_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			green_farm_create_settings_field($option);
		}
	}
...
function green_farm_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('green-farm-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('green-farm-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('green-farm-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (green_farm_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('green-farm-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'green-farm'), 
		'remove_sidebar' => 				esc_html__('Do you really want to remove this sidebar?', 'green-farm'), 
		'find' => 							esc_attr__('Find icons', 'green-farm'), 
		'remove_icon' => 					esc_html__('Do you really want to remove this social icon?', 'green-farm'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'green-farm', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'green-farm'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'green-farm'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'green-farm'), 
	...
	wp_enqueue_script('green-farm-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('green-farm-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'green-farm' 
	));
...
function green_farm_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'green-farm'), 
		esc_attr__('Theme Settings', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings', 
		'green_farm_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'green-farm'), 
		esc_attr__('General', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'green_farm_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'green-farm'), 
		esc_attr__('Fonts', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'green_farm_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'green-farm'), 
		esc_attr__('Colors', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'green_farm_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'green-farm'), 
		esc_attr__('Elements', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'green_farm_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'green-farm'), 
		esc_attr__('Single Posts', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'green_farm_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'green-farm'), 
		esc_attr__('Import/Export', 'green-farm'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'green_farm_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'green_farm_settings_scripts');
}


function green_farm_section_fn($desc) {
	$tab = green_farm_get_the_tab();
...
function green_farm_settings_page_fn() {
	$settings_output = green_farm_get_settings();
	$current_tab = green_farm_get_the_tab();
	
	echo '<div class="wrap">';
	
	green_farm_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page"' . (($current_tab == 'recaptcha') ? ' style="background-color:#fdffc6; padding:35px 0 15px 25px; margin:0;"' : '') . '>' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'green-farm') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'green-farm') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'green-farm') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'green-farm') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'green-farm') . '" />';
				}
...
function green_farm_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_green-farm_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_green-farm_theme_style">';
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (green_farm_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'green-farm') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'green-farm') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'green-farm') . ' <em>(' . esc_html__('in pixels', 'green-farm') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'green-farm') . ' <em>(' . esc_html__('in pixels', 'green-farm') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'green-farm') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'green-farm') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'green-farm') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (green_farm_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'green-farm') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'green-farm') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'green-farm') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'green-farm') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'green-farm') . '</span>' . 
					'<div>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'green-farm') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'green-farm') . '</em></span>' . 
					'<div>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'green-farm') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'green-farm') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'green-farm') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'green-farm') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'green-farm') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'green-farm') . '"></a> ' . 
...
function green_farm_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'green-farm');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'green-farm' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'green-farm'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'green-farm');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'green-farm' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','green-farm'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'green-farm');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'green-farm');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'green-farm' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'green-farm'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'green-farm');
...
function green_farm_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function green_farm_admin_msgs() {
	global $pagenow;
...
function green_farm_add_global_options($reset = false) {
	...
	foreach (green_farm_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			green_farm_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'green-farm' . '_social_icons' 
				) {
...
function green_farm_get_global_options() {
	...
	foreach (green_farm_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'green-farm' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}
