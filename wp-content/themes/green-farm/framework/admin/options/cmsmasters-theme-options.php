...
if (!function_exists('green_farm_get_custom_all_meta_fields')) {
function green_farm_get_custom_all_meta_fields() {
	$custom_meta_fields = green_farm_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = green_farm_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = green_farm_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = green_farm_get_custom_profile_meta_fields();
		...
		$custom_post_meta_fields = green_farm_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = green_farm_get_custom_other_meta_fields();
...
function green_farm_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_enqueue_style('green-farm-options-css', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options.css', array(), '1.0.0', 'screen');
		
		
		if (is_rtl()) {
			wp_enqueue_style('green-farm-options-css-rtl', get_template_directory_uri() . '/framework/admin/options/css/cmsmasters-theme-options-rtl.css', array(), '1.0.0', 'screen');
		}
		
		
		wp_enqueue_script('green-farm-options-js', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options.js', array('jquery'), '1.0.0', true);
		
		wp_localize_script('green-farm-options-js', 'cmsmasters_options', array( 
			'palettes' => 			implode(',', cmsmasters_color_picker_palettes()), 
			'create_gallery' => 	esc_attr__('Create Gallery', 'green-farm'), 
			'select_format' => 		esc_attr__('Please select the format.', 'green-farm'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'green-farm'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'green-farm'), 
		...
		wp_enqueue_script('green-farm-options-js-toggle', get_template_directory_uri() . '/framework/admin/options/js/cmsmasters-theme-options-toggle.js', array('jquery'), '1.0.0', true);
	}
}

add_action('admin_enqueue_scripts', 'green_farm_admin_enqueue_scripts');


function green_farm_show_meta_box() {
	global $post;
	
	$custom_all_meta_fields = green_farm_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'green-farm') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'green-farm') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'green-farm') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'green-farm') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'green-farm') . '</option>';
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'green-farm') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'green-farm') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'contact':
			...
			'<div class="contact_info_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Add info', 'green-farm') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon cmsmasters_remove_icon_contact_info admin-icon-remove" title="' . esc_attr__('Cancel changes', 'green-farm') . '"></a>' . 
				'</p>' . 
				...
				'</span>' . 
				'<span class="cl"></span>' . 
				'<input class="button button-primary" type="button" id="add_contact_info" value="' . esc_attr__('Add Contact Info', 'green-farm') . '" />' . 
				'<input class="button button-primary" type="button" id="edit_contact_info" value="' . esc_attr__('Save Contact Info', 'green-farm') . '" />' . 
				'<ul>';
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'green-farm') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'green-farm') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'green-farm') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'green-farm') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'green-farm') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'green-farm') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'green-farm') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'green-farm') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'green-farm') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'green-farm') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'green-farm') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'green-farm') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'green-farm') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function green_farm_save_custom_meta($post_id) {
    $custom_all_meta_fields = green_farm_get_custom_all_meta_fields();
...
add_action('save_post', 'green_farm_save_custom_meta');


function green_farm_add_custom_meta_box() {
	$args = array(
	   'public' => true 
	);
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'green-farm'), 
			'green_farm_show_meta_box', 
			$screen, 
			'normal', 
			'high' 
		);
...
add_action('add_meta_boxes', 'green_farm_add_custom_meta_box');
