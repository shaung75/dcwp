...
if (!function_exists('green_farm_system_fonts_list')) {
	function green_farm_system_fonts_list() {
		$fonts = array( 
...
if (!function_exists('green_farm_get_google_fonts_list')) {
	function green_farm_get_google_fonts_list() {
		$fonts = array( 
			'' => esc_html__('None', 'green-farm'), 
		...
		return apply_filters('green_farm_google_fonts_list_filter', $fonts);
	}
}
...
if (!function_exists('green_farm_text_decoration_list')) {
	function green_farm_text_decoration_list() {
		$list = array( 
			'none' => esc_html__('none', 'green-farm'), 
			'underline' => esc_html__('underline', 'green-farm'), 
			'overline' => esc_html__('overline', 'green-farm'), 
			'line-through' => esc_html__('line-through', 'green-farm'), 
		);
...
if (!function_exists('green_farm_custom_color_schemes_list')) {
	function green_farm_custom_color_schemes_list() {
		$list = array( 
			'first' => esc_html__('Custom 1', 'green-farm'), 
			'second' => esc_html__('Custom 2', 'green-farm'), 
			'third' => esc_html__('Custom 3', 'green-farm') 
		);
		
		
		return apply_filters('green_farm_custom_color_schemes_list_filter', $list);
	}
}
...
if (!function_exists('green_farm_locate_template')) {
	function green_farm_locate_template($template_names, $require_once = true, $load = true) {
		$located = '';
...
// Theme Style
define('CMSMASTERS_THEME_STYLE', (CMSMASTERS_THEME_STYLE_COMPATIBILITY && get_option('cmsmasters_green-farm_theme_style') ? get_option('cmsmasters_green-farm_theme_style') : ''));
...
if (!function_exists('green_farm_all_theme_styles')) {
	function green_farm_all_theme_styles() {
		$out = array( 
			'Default|', 
			'Theme Style 1|1', 
			'Theme Style 2|2' 
...
if (!function_exists('green_farm_all_color_schemes_list')) {
	function green_farm_all_color_schemes_list() {
		$list = array( 
			'default' => 		esc_html__('Default', 'green-farm'), 
			'header' => 		esc_html__('Header', 'green-farm'), 
			'navigation' => 	esc_html__('Navigation', 'green-farm'), 
			'header_top' => 	esc_html__('Header Top', 'green-farm'), 
			'footer' => 		esc_html__('Footer', 'green-farm') 
		);
		
		
		$out = array_merge($list, green_farm_custom_color_schemes_list());
...
if (CMSMASTERS_LEARNPRESS) {
	...
	function green_farm_change_template_for_learnpress() {
		return 'learnpress/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/templates';
	}
	
	add_filter('learn_press_template_path', 'green_farm_change_template_for_learnpress');
}
...
if (!function_exists('green_farm_load_theme_textdomain')) {
	function green_farm_load_theme_textdomain() {
		load_theme_textdomain('green-farm', get_template_directory() . '/theme-framework/languages');
	}
}

// Load Theme Local File Action
if (!has_action('after_setup_theme', 'green_farm_load_theme_textdomain')) {
	add_action('after_setup_theme', 'green_farm_load_theme_textdomain');
}



// Framework Activation & Data Import
if (!function_exists('green_farm_theme_activation')) {
	function green_farm_theme_activation() {
		if (get_option('cmsmasters_active_theme') != 'green-farm') {
			add_option('cmsmasters_active_theme', 'green-farm', '', 'yes');
...
add_action('after_switch_theme', 'green_farm_theme_activation');



// Framework Deactivation
if (!function_exists('green_farm_theme_deactivation')) {
	function green_farm_theme_deactivation() {
		delete_option('cmsmasters_active_theme');
	}
}

add_action('switch_theme', 'green_farm_theme_deactivation');



// Plugin Activation Regenerate Styles
if (!function_exists('green_farm_plugin_activation')) {
	function green_farm_plugin_activation() {
		update_option('cmsmasters_plugin_activation', 'true');
	}
}

add_action('activated_plugin', 'green_farm_plugin_activation');


if (!function_exists('green_farm_plugin_activation_regenerate')) {
	function green_farm_plugin_activation_regenerate() {
...
add_action('init', 'green_farm_plugin_activation_regenerate');
